...
@app.get('/products/sort_by_category')
...
def sort_products(sort_by: str = Query('price', description='price or name'),order:   str = Query('asc',   description='asc or desc'),):
...
def product_discount(category: str = Query(..., description='Category to discount'), discount_percent: int = Query(..., ge=1, le=99, description='% off'),):
    discount_updated=[]
    for p in products:
        if p['category'] == category:
            p['price'] = int(p['price']) * (1 - discount_percent / 100)
            discount_updated.append(p)
        if not discount_updated:
            return {'message': f'No products found in category: {category}'}
    return {'message': f'Products in category {category} discounted by {discount_percent}%', 'discounted_products': discount_updated}
...
def get_orders_by_search(customer_name: str = Query(...)):
...
def get_products_audit():
    ...
    total__stock_vakue=sum(p['price']*10 for p in in_stock)
    costliest=max(products, key=lambda p: p["price"])
    return {'total products':len(products), 'in_stock':len(in_stock), 'out_of_stock_names':[p['name'] for p in out_of_stock], 'total_stock_value':total__stock_vakue, 'most_expensive': {'name': costliest['name'], 'price': costliest['price']},}
